...
% J_stability = sum(sum(90*exp(-4*K_inst)./(max(K_inst,1e-6)))); % 45
J_stability = sum(sum(exp(-4*K_inst)./(max(K_inst,1e-2))));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if X_glo(1) > 1
    if X_glo(1) < x_obs_cur
        K_mv = ( (X_glo - x_obs).^2./d.^2 + (Y_glo+2.1).^2./b^2 ).^0.5 - 1;
        J_mv = sum(sum(exp(-0.1*K_mv)./max(K_mv,1e-2)));
    else
        K_mv = ( (X_glo - x_obs).^2./af.^2 + (Y_glo+2.1).^2./b^2 ).^0.5 - 1;
        J_mv = sum(sum(exp(-0.1*K_mv)./max(K_mv,1e-2)));
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
